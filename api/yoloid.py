...
def draw_prediction(img, class_id, confidence, x, y, x_plus_w, y_plus_h, ob):
    datas.append({
        "object": ob,
...
def img_detector(testimg):
    ...
    for i in indices:
        ...
        h = box[3]
        ob = ob + 1
        draw_prediction(image, class_ids[i], confidences[i], round(x), round(y), round(x+w), round(y+h), ob)
    #cv2.imshow("object detection", image)
    #cv2.waitKey()
    cv2.imwrite("opimg.jpg", image)
    return datas

#Getting Images From Directory :

if __name__ == "__main__":
    result = img_detector('test.jpg')
    image = cv2.imread("opimg.jpg")
    ims = cv2.resize(image, (1366, 768)) 
    cv2.imshow("Test Output", ims)
    cv2.waitKey()
    print("Images Processed : ",result)
